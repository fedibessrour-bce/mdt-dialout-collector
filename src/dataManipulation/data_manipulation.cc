    ...
    Json::Value &root)
{
    ...
    if (nokia_stream.has_update()) {
        ...
        // Log the content of nokia_stream.extension if there is one, but it seems like there isn't one
    ...
    if (!status.ok()) {
    spdlog::get("multi-logger")->error("[KafkaProducer] Failed to convert protobuf to JSON: {}", status.ToString());
    }
    // Log raw_data for debugging
    spdlog::get("multi-logger")->debug("[KafkaProducer] raw_data: {}", raw_data);
